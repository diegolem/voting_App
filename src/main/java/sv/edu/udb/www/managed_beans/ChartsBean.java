...
public class ChartsBean implements Serializable {
    ...
    public void graficaDepartamentos() {
        this.pieD = new PieChartModel();
        
        if (this.idRequest == 0) this.idRequest = Utilities.getRequestIntValue("idRequest");
    ...
    public void graficaMunicipios() {
        ...
        if (this.idRequest == 0) this.idRequest = Utilities.getRequestIntValue("idRequest");
        if (this.idD == 0) this.idD = Utilities.getRequestIntValue("dep");
        
        if (this.idD == 0){
            List<Departments> deps = this.getDepartmentsC();
            if (deps.size() > 0) this.idD = deps.get(0).getId();
        }
        
        if (this.idRequest != 0 && this.idD != 0) {
...
//        Departments _de = departmentsModel.getDepartment(this.idD);
    ...
    public void graficaSedes() {
        ...
        if (this.idRequest == 0) this.idRequest = Utilities.getRequestIntValue("idRequest");
        if (this.idC == 0) this.idC = Utilities.getRequestIntValue("sede");
        if (this.idC == 0){
            List<Cities> cities = this.getCitiesC();
            if (cities.size() > 0) this.idC = cities.get(0).getId();
        }
        
        if (this.idRequest != 0 && this.idC != 0) {
            ElectoralProcess _ep = electoralProcessModel.getElectoralProcess(this.idRequest);
            List<Integer> listaVotos = new ArrayList();
    ...
    public void graficaJrv() {
        ...
        if (this.idRequest == 0) this.idRequest = Utilities.getRequestIntValue("idRequest");
        if (this.idS == 0) this.idS = Utilities.getRequestIntValue("jrv");
        
        if (this.idS == 0){
            List<Headquarters> hdts = this.getHeadquarterC();
            if (hdts.size() > 0) this.idS = hdts.get(0).getId();
        }
        
        if (this.idRequest != 0 && this.idS != 0) {
